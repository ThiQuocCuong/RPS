...
//	retryTimer = boost::shared_ptr<boost::asio::deadline_timer>(new boost::asio::deadline_timer(*(this->GetBoostService())));
	io_service_retryTimer = boost::shared_ptr<boost::asio::io_service>(new boost::asio::io_service());
	retryTimer = boost::shared_ptr<boost::asio::deadline_timer>(new boost::asio::deadline_timer(*io_service_retryTimer));
}
		
// -------------------------------------------------------------------
// Constructor
// -------------------------------------------------------------------
BitSwarmClient::BitSwarmClient(boost::shared_ptr<SmartFox> sfs) 
{
	this->sfs = sfs;
	log = sfs->Log();
	socket = boost::shared_ptr<ISocketLayer>();
...
void BitSwarmClient::Dispose()
{
	...
	{
		retryTimer->cancel();
		retryTimer = boost::shared_ptr<boost::asio::deadline_timer>();
	}
	...
	std::map<long int, boost::shared_ptr<IController> >::iterator iterator;
...
// CryptoKey
// -------------------------------------------------------------------
boost::shared_ptr<Sfs2X::Util::CryptoKey> BitSwarmClient::CryptoKey()
{
	return cryptoKey;
}
...
// CryptoKey
// -------------------------------------------------------------------
void BitSwarmClient::CryptoKey(boost::shared_ptr<Sfs2X::Util::CryptoKey> value)
{	
	// TODO Need to check if already set?
	cryptoKey = value;
}
...
//	retryTimer->async_wait(boost::bind(&BitSwarmClient::OnRetryConnectionEvent, this, boost::asio::placeholders::error));
	io_service_retryTimer->reset();
	retryTimer->async_wait(boost::bind(&BitSwarmClient::OnRetryConnectionEvent, this, boost::asio::placeholders::error));
	io_service_retryTimer->run();
}
...
void BitSwarmClient::OnRetryConnectionEvent(const boost::system::error_code& code)
{
	...
	// Here we expect that socket is not connected
	// Force socket disconnection when it has an unexpected status
    if (socket->IsConnected()) 
	{
		socket->Disconnect();
    }

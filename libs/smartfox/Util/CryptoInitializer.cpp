...
void CryptoInitializer::RunHelper()
{
	...
	*/
	...
	boost::shared_future<bool> f = boost::async(boost::launch::async, boost::bind((useHttps ? &CryptoInitializer::RunHelperSSLAsync : &CryptoInitializer::RunHelperAsync), this));
...
bool CryptoInitializer::RunHelperAsync()
{
	...
	try 
	{
		...
		boost::shared_ptr<IPAddress> address (new IPAddress(IPAddress::IPADDRESSTYPE_IPV4, sfs->Config()->Host()));
	...
	try
	{
		...
		if (parts.size() < 2) {
			OnHttpError(boost::shared_ptr<string>(new string ("Error during http response: connection closed by remote side")));	
			return returned;
		}

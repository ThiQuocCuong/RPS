...
// Dispose
// -------------------------------------------------------------------
void ConfigLoader::Dispose()
{
	this->smartFox = boost::shared_ptr<SmartFox>();
...
void ConfigLoader::LoadConfig(string filePath)
{
	...
	// allocate memory:
	boost::shared_ptr<char> fileText (new char [length], array_deleter<char>());
...
void ConfigLoader::TryParse()
{
	...
	try 
	{
		...
		if ((GetNodeText(configNode, "httpsPort") != NULL) && ((*(GetNodeText(configNode, "httpsPort"))) != "")) {
			cfgData->HttpsPort(atoi(GetNodeText(configNode, "httpsPort")->c_str()));
		}
